...
ggplot(ranked, aes(x=as.factor(rank), y=nRight, color=type, group=type)) + 
  geom_line() + 
  geom_point() + 
  jtools::theme_apa() + 
  scale_x_discrete(name="Model Probability", labels = paste0(seq(10,100,by=10),"%")) + 
  scale_y_continuous(name="Choice Probability", breaks=seq(.10,1,by=.10), labels = paste0(seq(10,100,by=10),"%") )
```

```{r}
APbins <- fullTD %>% mutate(rank=ntile(fullTD$AP,10))
DESbins <- fullTD %>% mutate(rank=ntile(fullTD$desAP,10))
FWSbins <- fullTD %>% mutate(rank=ntile(fullTD$SV.AP,10))

APranked <- APbins %>% group_by(compMotC3, rank) %>% summarise(nRight = mean(chooseDes), nLeft = 1-mean(chooseDes))
APranked$type <- "AP"
DESranked <- DESbins %>% group_by(compMotC3, rank) %>% summarise(nRight = mean(chooseDes), nLeft = 1-mean(chooseDes))
DESranked$type <- "DES"
FWSranked <- FWSbins %>% group_by(compMotC3, rank) %>% summarise(nRight = mean(chooseDes), nLeft = 1-mean(chooseDes))
FWSranked$type <- "FWS"
ranked <-rbind(APranked,DESranked,FWSranked)
...
```{r}
m <- glmer( as.factor(choice) ~ scale(runSumRLDiff)*incong + scale(trialTotal) + ( scale(trialTotal) + scale(runSumRLDiff) + incong | subID), data = fullTD2, family = binomial, control = glmerControl(optimizer = "bobyqa",
                                    optCtrl = list(maxfun = 100000)),
    nAGQ = 1)
  summary(m)
  tidy(m,conf.int=TRUE,exponentiate=TRUE,effects="fixed")
...
```{r}
m <- glmer( as.factor(choice) ~ scale(runSumRLDiff)*incong + scale(trialTotal) + ( scale(trialTotal) + scale(runSumRLDiff) + incong | subID), data = fullTD3, family = binomial, control = glmerControl(optimizer = "bobyqa",
                                    optCtrl = list(maxfun = 100000)),
    nAGQ = 1)
summary(m)
tidy(m,conf.int=TRUE,exponentiate=TRUE,effects="fixed")
...
```{r}
m <- glmer(  as.factor(choice) ~ scale(runSumRLDiff)*condition + scale(trialTotal) + ( scale(trialTotal) + scale(runSumRLDiff) + scale(propRminL) | subID), data = fullTD3, family = binomial, control = glmerControl(optimizer = "bobyqa",
                                    optCtrl = list(maxfun = 100000)),
    nAGQ = 1)
summary(m)
tidy(m,conf.int=TRUE,exponentiate=TRUE,effects="fixed")
...
```{r}
m <- lmer( scale(selfResp) ~ scale(sumCueF) + scale(desirability) +
    ( scale(sumCueF)  + scale(desirability) | subID) + (1 | trait), data = reValDf2, control = lmerControl(optimizer = "bobyqa",
                                    optCtrl = list(maxfun = 100000)))
summary(m)
tidy(m,conf.int=TRUE,effects="fixed")
```
