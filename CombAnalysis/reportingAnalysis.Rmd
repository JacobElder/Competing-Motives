...
APbins <- fullTD %>% mutate(rank=ntile(fullTD$rAP,10))
DESbins <- fullTD %>% mutate(rank=ntile(fullTD$rightDesAP,10))
FWSbins <- fullTD %>% mutate(rank=ntile(fullTD$RSV.AP,10))
...
ggplot(APranked, aes(x=as.factor(rank), y=nRight)) + geom_point()

ggplot(ranked, aes(x=as.factor(rank), y=nRight, color=type)) + geom_point() +jtools::theme_apa()
```
...
```{r}
m <- lmer( desAP ~ scale(trialTotal)*condition + ( scale(trialTotal) | subID) + (1 | traitLeft) + (1 |traitRight), data = fullTD3)
summary(m)
```
...
```{r}
m <- glmer( choice ~ scale(trialTotal) + scale(runAveRLDiff) + ( scale(trialTotal) + scale(runAveRLDiff) | subID)  + (1 | traitLeft) + (1 |traitRight), data = fullTD1, family = binomial, control = glmerControl(optimizer = "bobyqa",
                                    optCtrl = list(maxfun = 100000)),
    nAGQ = 1)
summary(m)
tidy(m,conf.int=TRUE,exponentiate=TRUE,effects="fixed")
...
```{r}
m <- glmer( choice ~ scale(propRminL) + scale(runSumRLDiff) + ( scale(trialTotal) + scale(runSumRLDiff) | subID), data = fullTD1, family = binomial, control = glmerControl(optimizer = "bobyqa",
                                    optCtrl = list(maxfun = 100000)),
    nAGQ = 1)
summary(m)
tidy(m,conf.int=TRUE,exponentiate=TRUE,effects="fixed")
...
```{r}
m <- glmer( choice ~ scale(trialTotal) + scale(runSumRLDiff)*incong + ( scale(trialTotal) + scale(runSumRLDiff) + incong | subID), data = fullTD1, family = binomial, control = glmerControl(optimizer = "bobyqa",
                                    optCtrl = list(maxfun = 100000)),
    nAGQ = 1)
  summary(m)
  tidy(m,conf.int=TRUE,exponentiate=TRUE,effects="fixed")
  ggpredict(m, c("runSumRLDiff","incong")) %>% plot()
```

```{r}
m <- lmer( scale(sumC.AP) ~ scale(affPol)*condition + ( 1 | subID), data = fullTD3)
summary(m)
```
...
```{r}
m <- glmer( as.factor(choice) ~ scale(propRminL) + scale(runSumRLDiff) + ( scale(trialTotal) + scale(runSumRLDiff) | subID), data = fullTD2, family = binomial, control = glmerControl(optimizer = "bobyqa",
                                    optCtrl = list(maxfun = 100000)),
    nAGQ = 1)
summary(m)
tidy(m,conf.int=TRUE,exponentiate=TRUE,effects="fixed")
...
```{r}
m <- glmer( as.factor(choice) ~ scale(propRminL) + scale(runSumRLDiff)*incong + ( scale(trialTotal) + scale(runSumRLDiff) + incong | subID), data = fullTD2, family = binomial, control = glmerControl(optimizer = "bobyqa",
                                    optCtrl = list(maxfun = 100000)),
    nAGQ = 1)
  summary(m)
  ggpredict(m, c("runSumRLDiff","incong")) %>% plot()
...
```{r}
m <- glmer(  as.factor(choice) ~ scale(propRminL) + scale(runSumRLDiff) + ( scale(trialTotal) + scale(runSumRLDiff) + incong | subID), data = fullTD3, family = binomial, control = glmerControl(optimizer = "bobyqa",
                                    optCtrl = list(maxfun = 100000)),
    nAGQ = 1)
summary(m)
tidy(m,conf.int=TRUE,exponentiate=TRUE,effects="fixed")
...
```{r}
m <- glmer( as.factor(choice) ~ scale(propRminL) + scale(runSumRLDiff)*incong + ( scale(trialTotal) + scale(runSumRLDiff) + incong | subID), data = fullTD3, family = binomial, control = glmerControl(optimizer = "bobyqa",
                                    optCtrl = list(maxfun = 100000)),
    nAGQ = 1)
summary(m)
ggpredict(m, c("runSumRLDiff","incong")) %>% plot()
...
```{r}
m <- glmer(  as.factor(choice) ~ scale(propRminL) + scale(runSumRLDiff)*condition + ( scale(trialTotal) + scale(runSumRLDiff) | subID), data = fullTD3, family = binomial, control = glmerControl(optimizer = "bobyqa",
                                    optCtrl = list(maxfun = 100000)),
    nAGQ = 1)
summary(m)
```
...
```{r}
m <- glmer( chooseDes ~ compMotC3 + scale(trialTotal) + ( compMotC3 + scale(trialTotal) | subID), data = incongDf1, family = binomial, control = glmerControl(optimizer = "bobyqa",
                                    optCtrl = list(maxfun = 100000)),
    nAGQ = 1)
summary(m)
tidy(m,conf.int=TRUE,exponentiate=TRUE,effects="fixed")
...
```{r}
m <- lmer(scale(sumC.AP) ~ scale(SE) * compMotC3 + ( compMotC3 | subID) + ( 1 | traitLeft) + (1 | traitRight), data = fullTD1)
summary(m)
ggpredict(m, c("SE","compMotC3")) %>% plot(show.title=F) + xlab("Self-Esteem") + ylab("Likelihood of Group Normativity") + jtools::theme_apa()
```
...
```{r}
m <- glmer( chooseDes ~ compMotC3 + scale(trialTotal) + ( compMotC3 + scale(trialTotal) | subID), data = incongDf2, family = binomial, control = glmerControl(optimizer = "bobyqa",
                                    optCtrl = list(maxfun = 100000)),
    nAGQ = 1)
summary(m)
ggpredict(m, c("compMotC3")) %>% plot()
...
```{r}
m <- lmer(scale(sumC.AP) ~ scale(SE) * compMotC3 + ( compMotC3 | subID) + ( 1 | traitLeft) + (1 | traitRight), data = fullTD2)
summary(m)
ggpredict(m, c("SE","compMotC3")) %>% plot(show.title=F) + xlab("Self-Esteem") + ylab("Likelihood of Group Normativity") + jtools::theme_apa()
```
...
```{r}
m <- glmer( chooseDes ~ compMotC3 + scale(trialTotal) + ( compMotC3 + scale(trialTotal) | subID), data = incongDf3, family = binomial, control = glmerControl(optimizer = "bobyqa",
                                    optCtrl = list(maxfun = 100000)),
    nAGQ = 1)
summary(m)
```
...
```{r}
m <- glmer( as.factor(choice) ~ scale(sumRminL) * scale(desRminL) * condition + incong + scale(propRminL) + ( scale(sumRminL) + scale(desRminL) | subID), data = fullTD3, family = binomial, control = glmerControl(optimizer = "bobyqa",
                                    optCtrl = list(maxfun = 100000)),
    nAGQ = 1)
summary(m)
ggpredict(m, c("sumRminL", "desRminL")) %>% plot()
```
...
```{r}
m <- glmer( chooseDes ~ condition*scale(simEnt) + ( scale(aveEnt) | subID), data = fullTD3, family = binomial, control = glmerControl(optimizer = "bobyqa",
                                    optCtrl = list(maxfun = 100000)),
    nAGQ = 1)
summary(m)
ggpredict(m, c("sumEnt","condition")) %>% plot()

m <- lmer(scale(desAP) ~ scale(sumEnt) * condition + ( 1| subID), data = fullTD3)
summary(m)
```
#### Moderated by Self-Esteem

```{r}
m <- lmer(scale(sumC.AP) ~ scale(SE) * compMotC3 * condition + ( compMotC3 | subID) + ( 1 | traitLeft) + (1 | traitRight), data = fullTD3)
summary(m)
...
```{r}
m <- lmer( desAP ~ compMotC3 + scale(trialTotal) + scale(trialTotal) + ( compMotC3 + scale(trialTotal) | subID), data = fullTD3)
summary(m)
...
summary(m)
r2beta(m,data=fullTD3)
ggpredict(m, c("SimRminL","condition")) %>% plot()
```
...
m <- lmer( scale(selfResp) ~ scale(sumCueF) * scale(propCueF) +
    (  scale(sumCueF) + scale(propCueF) | subID) + (1 | trait), data = reValDf2, control = lmerControl(optimizer = "bobyqa",
                                    optCtrl = list(maxfun = 100000)))
summary(m)
ggpredict(m, c("sumCueF","propCueF")) %>% plot()
```
...
```{r}
m <- lmer( scale(selfResp) ~ scale(sumCueF) +
    (  scale(sumCueF) | subID) + (1 | trait), data = reValDf3, control = lmerControl(optimizer = "bobyqa",
                                    optCtrl = list(maxfun = 100000)))
summary(m)
ggpredict(m, c("sumCueF")) %>% plot()
```
...
```{r}
m <- lmer( scale(selfResp) ~ scale(sumCueF) * condition +
    ( scale(sumCueF) | subID) + ( condition | trait), data = reValDf3, control = lmerControl(optimizer = "bobyqa",
                                    optCtrl = list(maxfun = 100000)))
summary(m)
ggpredict(m, c("sumCueF","condition")) %>% plot()
```
...
```{r}
m <- lmer( scale(selfResp) ~ scale(sumCueF) * valence +
    ( scale(sumCueF) + valence | subID) + ( 1 | trait), data = reValDf3, control = lmerControl(optimizer = "bobyqa",
                                    optCtrl = list(maxfun = 100000)))
summary(m)
ggpredict(m, c("sumCueF","valence")) %>% plot()
```
...
```{r}
m <- lmer(selfResp ~ SV_F*condition*scale(affPol) + desirability +
    ( SV_F + desirability | subID) + (1 | trait), data = reValDf3, control = lmerControl(optimizer = "bobyqa",
                                    optCtrl = list(maxfun = 100000)))
summary(m)
ggpredict(m, c("SV_F")) %>% plot()
...
m <- lmer(scale(selfResp) ~ scale(SV_F)*as.factor(unseen) + scale(desirability) +
    ( scale(SV_F) + as.factor(unseen) | subID) + (1 | trait), data = reValDf2, control = lmerControl(optimizer = "bobyqa",
                                    optCtrl = list(maxfun = 100000)))
summary(m)
...
```{r}
m <- lmer(scale(selfResp) ~ scale(SV_F)*as.factor(unseen) + scale(desirability) +
    ( scale(SV_F) + as.factor(unseen) | subID) + (1 | trait), data = reValDf3, control = lmerControl(optimizer = "bobyqa",
                                    optCtrl = list(maxfun = 100000)))
summary(m)
ggpredict(m, c("SV_F", "unseen" )) %>% plot()
```
