...
### Generalization to Novel Traits
...
simple effects of similarity-to-group
...
```{r}
m <- lmer(scale(selfResp) ~ scale(sumCueF)*as.factor(unseen) + scale(Sim) + scale(desirability) +
    ( scale(sumCueF) + scale(Sim) + scale(desirability) + as.factor(unseen) | subID) + (1 | trait), data = reValDf2, control = lmerControl(optimizer = "bobyqa",
                                    optCtrl = list(maxfun = 100000)))
summary(m)
tidy(m,conf.int=TRUE,effects="fixed")
parameters::equivalence_test(m)
```
simple effects of summed probs

```{r}
m <- lmer(scale(selfResp) ~ scale(sumCueF) + scale(Sim) + scale(desirability) +
    ( scale(sumCueF) + scale(Sim) + scale(desirability)  | subID) + (1 | trait), data = n2, control = lmerControl(optimizer = "bobyqa",
                                    optCtrl = list(maxfun = 100000)))
summary(m)
tidy(m,conf.int=TRUE,effects="fixed")
data.frame(r2beta(m))
...
m <- lmer(scale(selfResp) ~ scale(sumCueF) + scale(Sim) + scale(desirability) +
    ( scale(sumCueF) + scale(Sim) + scale(desirability) | subID) + (1 | trait), data = p2, control = lmerControl(optimizer = "bobyqa",
                                    optCtrl = list(maxfun = 100000)))
summary(m)
tidy(m,conf.int=TRUE,effects="fixed")
data.frame(r2beta(m))
...
```{r}
m <- glmer( as.factor(choice) ~ scale(RSV) + scale(desRminL) + ( scale(RSV) + scale(desRminL) | subID), data = fullTD3, family = binomial, control = glmerControl(optimizer = "bobyqa",
                                    optCtrl = list(maxfun = 100000)),
    nAGQ = 1)
summary(m)
tidy(m,conf.int=TRUE,exponentiate=TRUE,effects="fixed")
...
des and sim-to-group by condiition

```{r}
m <- glmer( as.factor(choice) ~ scale(RSV)*condition + scale(desRminL) + ( scale(RSV) + scale(desRminL) | subID), data = fullTD3, family = binomial, control = glmerControl(optimizer = "bobyqa",
                                    optCtrl = list(maxfun = 100000)),
    nAGQ = 1)
summary(m)
tidy(m,conf.int=TRUE,exponentiate=TRUE,effects="fixed")
...
```{r}
m <- glmer( as.factor(choice) ~ scale(RSV)*scale(SE)*condition + scale(desRminL)*scale(SE)*condition + ( scale(RSV) + scale(desRminL) | subID), data = fullTD3, family = binomial, control = glmerControl(optimizer = "bobyqa",
                                    optCtrl = list(maxfun = 100000)),
    nAGQ = 1)
summary(m)
tidy(m,conf.int=TRUE,exponentiate=TRUE,effects="fixed")
data.frame(r2beta(m,data=fullTD3[complete.cases(fullTD3),]))
```
identification moderating

```{r}
m <- glmer( as.factor(choice) ~ scale(RSV)*scale(IdImp)*condition + scale(desRminL)*scale(IdImp)*condition + ( scale(RSV) + scale(desRminL) | subID), data = fullTD3, family = binomial, control = glmerControl(optimizer = "bobyqa",
                                    optCtrl = list(maxfun = 100000)),
    nAGQ = 1)
summary(m)
tidy(m,conf.int=TRUE,exponentiate=TRUE,effects="fixed")
...
```{r}
m <- glmer( as.factor(choice) ~ scale(runAveRLDiff) + scale(SimRminL) + scale(desRminL) + ( scale(runAveRLDiff) + scale(SimRminL) + scale(desRminL) | subID), data = fullTD3, family = binomial, control = glmerControl(optimizer = "bobyqa",
                                    optCtrl = list(maxfun = 100000)),
    nAGQ = 1)
summary(m)
tidy(m,conf.int=TRUE,exponentiate=TRUE,effects="fixed")
...
```{r}
m <- glmer( as.factor(choice) ~ scale(runSumRLDiff) + scale(SimRminL) + scale(desRminL) + ( scale(runSumRLDiff) + scale(SimRminL) + scale(desRminL) | subID), data = fullTD3, family = binomial, control = glmerControl(optimizer = "bobyqa",
                                    optCtrl = list(maxfun = 100000)),
    nAGQ = 1)
summary(m)
tidy(m,conf.int=TRUE,exponentiate=TRUE,effects="fixed")
...
des, sim, sum prob by condition

```{r}
m <- glmer( as.factor(choice) ~ scale(runSumRLDiff)*condition + scale(SimRminL) + scale(desRminL) + ( scale(runSumRLDiff) + scale(SimRminL) + scale(desRminL) | subID), data = fullTD3, family = binomial, control = glmerControl(optimizer = "bobyqa",
                                    optCtrl = list(maxfun = 100000)),
    nAGQ = 1)
summary(m)
tidy(m,conf.int=TRUE,exponentiate=TRUE,effects="fixed")
...
```{r}
m <- glmer( as.factor(chooseDes) ~ compMotC3 + cong + scale(trialTotal) + ( compMotC3 + scale(trialTotal) | subID), data = fullTD3, family=binomial)
summary(m)
tidy(m,conf.int=TRUE,exponentiate=TRUE,effects="fixed")
data.frame(r2beta(m,data=fullTD3))
```

compete by condition

```{r}
m <- glmer( as.factor(chooseDes) ~ compMotC3*condition + cong + scale(trialTotal) + ( compMotC3 + scale(trialTotal) | subID), data = fullTD3, family=binomial)
summary(m)
tidy(m,conf.int=TRUE,exponentiate=TRUE,effects="fixed")
data.frame(r2beta(m,data=fullTD3))
```
...
```{r}
m <- glmer( as.factor(chooseDes) ~ compMotC3*scale(SE)*condition + cong + scale(trialTotal) + ( compMotC3 | subID), data = fullTD3, family=binomial)
summary(m)
tidy(m,conf.int=TRUE,exponentiate=TRUE,effects="fixed")
data.frame(r2beta(m,data=fullTD3[complete.cases(fullTD3),]))
```
...
```{r}
m <- glmer( as.factor(chooseDes) ~ compMotC3*scale(IdImp)*condition + cong + scale(trialTotal) + ( compMotC3 | subID), data = fullTD3, family=binomial)
summary(m)
tidy(m,conf.int=TRUE,exponentiate=TRUE,effects="fixed")
data.frame(r2beta(m,data=fullTD3[complete.cases(fullTD3),]))
```
...
```{r}
m <- lmer(selfResp ~ scale(valEstF) * condition +
    ( scale(valEstF) | subID) + (1 | trait), data = reValDf3, control = lmerControl(optimizer = "bobyqa",
                                    optCtrl = list(maxfun = 100000)))
summary(m)
tidy(m,conf.int=TRUE,effects="fixed")
...
des and sim-to-group by condition
...
cumulative probabilities by condition
...
```{r}
n3 <- subset(reValDf3, unseen==1)
p3 <- subset(reValDf3, unseen==0)

m <- lmer(scale(selfResp) ~ scale(SV_F) + scale(desirability) +
    ( scale(SV_F) + scale(desirability) | subID) + (1 | trait), data = n3, control = lmerControl(optimizer = "bobyqa",
                                    optCtrl = list(maxfun = 100000)))
summary(m)
tidy(m,conf.int=TRUE,effects="fixed")
r2beta(m)
...
m <- lmer(scale(selfResp) ~ scale(SV_F) + scale(desirability) +
    ( scale(SV_F) + scale(desirability) | subID) + (1 | trait), data = p3, control = lmerControl(optimizer = "bobyqa",
                                    optCtrl = list(maxfun = 100000)))
summary(m)
tidy(m,conf.int=TRUE,effects="fixed")
r2beta(m)
...
simple effects of similarity-to-group by condition
...
```{r}
m <- lmer(scale(selfResp) ~ scale(sumCueF)*as.factor(unseen) + scale(Sim) + scale(desirability) +
    ( scale(sumCueF) + scale(Sim) + scale(desirability) + as.factor(unseen) | subID) + (1 | trait), data = reValDf3, control = lmerControl(optimizer = "bobyqa",
                                    optCtrl = list(maxfun = 100000)))
summary(m)
tidy(m,conf.int=TRUE,effects="fixed")
parameters::equivalence_test(m)
...
```{r}
m <- lmer(scale(selfResp) ~ scale(sumCueF) + scale(Sim) + scale(desirability) +
    ( scale(sumCueF) + scale(Sim) + scale(desirability)  | subID) + (1 | trait), data = n3, control = lmerControl(optimizer = "bobyqa",
                                    optCtrl = list(maxfun = 100000)))
summary(m)
tidy(m,conf.int=TRUE,effects="fixed")
data.frame(r2beta(m))
...
m <- lmer(scale(selfResp) ~ scale(sumCueF) + scale(Sim) + scale(desirability) +
    ( scale(sumCueF) + scale(Sim) + scale(desirability) | subID) + (1 | trait), data = p3, control = lmerControl(optimizer = "bobyqa",
                                    optCtrl = list(maxfun = 100000)))
summary(m)
tidy(m,conf.int=TRUE,effects="fixed")
data.frame(r2beta(m))
